...
export const generatePolicyPDF = (record: VehicleServiceRecord): void => {
  ...
  // Page 1 - Policy Certificate and Invoice
  ...
  doc.text('MARUTI SUZUKI', 105, 30, { align: 'center' });
  doc.setFontSize(12);
  doc.setFont('helvetica', 'normal');
  doc.text('24x7 ROADSIDE ASSISTANCE PROGRAM', 105, 36, { align: 'center' });
  ...
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.text('Certificate Number:', 20, yPos + 3);
  doc.setFont('helvetica', 'normal');
  doc.text(certificateNumber, 80, yPos + 3);
  doc.setFont('helvetica', 'bold');
  doc.text('Issue Date:', 130, yPos + 3);
  doc.setFont('helvetica', 'normal');
  doc.text(format(new Date(), 'dd/MM/yyyy'), 160, yPos + 3);
  yPos += 25;
  ...
  policyHolderData.forEach(([label, value], index) => {
    ...
    doc.rect(20, rowY - 2, 160, 8);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(0, 0, 0);
    doc.text(label, 25, rowY + 3);
    doc.setFont('helvetica', 'normal');
    const lines = doc.splitTextToSize(value, 100);
    doc.text(lines[0], 80, rowY + 3);
    if (lines.length > 1) {
      doc.text(lines[1], 80, rowY + 8);
    }
  });
  yPos += (policyHolderData.length * 8) + 15;
  ...
  vehicleData.forEach(([label, value], index) => {
    ...
    doc.text(label, 25, rowY + 3);
    doc.setFont('helvetica', 'normal');
    doc.text(value, 80, rowY + 3);
  });
  yPos += (vehicleData.length * 8) + 15;
  ...
  yPos += 6;
  doc.text(record.registeredAddress, 20, yPos);
  yPos += 6;
  ...
  yPos += 6;
  doc.text(record.customerAddress, 20, yPos);
  yPos += 6;
  ...
  // Table rows
  const invoiceRows = [
    ['24x7 Roadside Assistance - ' + record.assist, `₹${basicPrice.toFixed(2)}`],
    ['CGST (9%)', `₹${cgst.toFixed(2)}`],
    ['SGST (9%)', `₹${sgst.toFixed(2)}`],
  ];
  
  invoiceRows.forEach(([desc, amount]) => {
